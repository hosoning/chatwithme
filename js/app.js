...
/* ============ 小程式：附近外卖 ============ */
const TEA_STORES=[
 {id:'mutea',name:'沐茶 MUTEA',short:'鲜果茶 · 轻乳茶 · 原叶茶',emoji:'🧋',color:'#15291f',mark:'MUTEA',meta:'元朗形点店 · 营业至 22:30',eta:'25–35 分钟',freeAt:38,categories:['人气推荐','轻乳茶','鲜果茶','原叶纯茶']},
 {id:'morning',name:'晨光早餐铺',short:'港式早餐 · 三明治 · 粥粉面',emoji:'🥪',color:'#d98539',mark:'MORNING',meta:'现点现做 · 营业至 11:30',eta:'20–30 分钟',freeAt:32,categories:['招牌早餐','三明治','粥粉面']},
 {id:'bistro',name:'小岛西餐厅',short:'意面 · 汉堡 · 扒类',emoji:'🍝',color:'#8a3e32',mark:'BISTRO',meta:'意面与手作汉堡 · 营业至 22:00',eta:'30–45 分钟',freeAt:58,categories:['主厨推荐','意面','汉堡']},
 {id:'kitchen',name:'家常小馆',short:'家常菜 · 一人食 · 炖汤',emoji:'🍚',color:'#b44e32',mark:'KITCHEN',meta:'热菜现炒 · 营业至 21:30',eta:'30–40 分钟',freeAt:48,categories:['下饭热菜','一人食','汤羹']},
 {id:'sweet',name:'云朵甜品屋',short:'蛋糕 · 糖水 · 每日烘焙',emoji:'🍰',color:'#bc7185',mark:'SWEET',meta:'每日新鲜烘焙 · 营业至 22:00',eta:'25–35 分钟',freeAt:42,categories:['人气甜点','蛋糕','糖水']}
];
const TEA_MENU=[
...
let _teaDraft={cart:{},storeId:'mutea',method:'delivery',recipientName:'',recipientPhone:'',address:'',note:''},_teaCustomProductId=null,_teaTrackTimers=[];
...
function renderTeaStore(){const s=currentTeaStore();document.getElementById('teaNavTitle').textContent=s.name;document.getElementById('teaStoreName').textContent=s.name;document.getElementById('teaStoreMeta').innerHTML=`${escapeHtml(s.meta)}<br>预计 ${escapeHtml(s.eta)}送达`;document.getElementById('teaStoreBadge').textContent=`满 ¥${s.freeAt} 免配送费`;document.getElementById('teaStoreHero').dataset.mark=s.mark;document.getElementById('teaStoreHero').style.background=s.color;document.getElementById('teaCategoryList').innerHTML=s.categories.map((x,i)=>`<button data-tea-category="${x}" class="${i===0?'selected':''}">${x}</button>`).join('');renderTeaMenu(s.categories[0]);}
...
function bindTeaOrder(){
 ...
 document.getElementById('teaConfirmPayBtn')?.addEventListener('click',()=>{if(_walletActionBusy)return;const total=teaTotal(),w=getWallet();if(w.balance<total){alert('余额不足，请先充值');return;}_walletActionBusy=true;runFaceIdSimulation(()=>{const w2=getWallet();if(w2.balance<total){_walletActionBusy=false;alert('余额不足');popPage();return;}const now=Date.now(),accept=25000+secureRandomInt(65000),prepare=(6+secureRandomInt(9))*60000,travel=_teaDraft.method==='delivery'?(12+secureRandomInt(17))*60000:(2+secureRandomInt(5))*60000,s=currentTeaStore(),matched=state.contacts.find(c=>String(c.name).trim()===_teaDraft.recipientName);const order={id:now,contactId:matched?.id||null,storeId:s.id,storeName:s.name,direction:'outgoing',items:teaCartItems(),method:_teaDraft.method,recipientName:_teaDraft.recipientName,recipientPhone:_teaDraft.recipientPhone,address:_teaDraft.address,note:_teaDraft.note,total,createdAt:now,acceptedAt:now+accept,preparedAt:now+accept+prepare,completedAt:now+accept+prepare+travel,notified:false};w2.balance-=total;w2.transactions.push({title:`${s.name}订单 · ${order.items.length}件`,amount:-total,ts:now});saveWallet(w2);safeSaveJSON(teaOrderKey(s.id),order);scheduleTeaDeliveryNotification(order);_walletActionBusy=false;popPage();renderTeaTracking(order);});});
...
function renderTeaTracking(order){
 ...
 document.getElementById('teaOrderSummary').innerHTML=`<strong>${escapeHtml(order.storeName||currentTeaStore(order.storeId).name)}</strong>${order.items.map(x=>`<span>${escapeHtml(teaItemDescription(x))}</span>`).join('')}<small>${order.direction==='incoming'?'对方已付款':'实付 ¥'+Number(order.total).toFixed(2)} · ${order.method==='delivery'?'配送给 '+escapeHtml(order.recipientName||'收货人')+' · '+escapeHtml(order.address||''):'到店自取'}</small>`;
