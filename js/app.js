...
/* ============ 小程式：附近外卖 ============ */
const TEA_STORES=[
 {id:'mutea',name:'沐茶 MUTEA',short:'奶茶',emoji:'🧋',mark:'MUTEA',meta:'元朗形点店 · 营业至 22:30',eta:'25–35 分钟',freeAt:38,categories:['人气推荐','轻乳茶','鲜果茶','原叶纯茶']},
 {id:'morning',name:'晨光早餐铺',short:'早餐',emoji:'🥪',mark:'MORNING',meta:'现点现做 · 营业至 11:30',eta:'20–30 分钟',freeAt:32,categories:['招牌早餐','三明治','粥粉面']},
 {id:'bistro',name:'小岛西餐厅',short:'西餐',emoji:'🍝',mark:'BISTRO',meta:'意面与手作汉堡 · 营业至 22:00',eta:'30–45 分钟',freeAt:58,categories:['主厨推荐','意面','汉堡']},
 {id:'kitchen',name:'家常小馆',short:'中餐',emoji:'🍚',mark:'KITCHEN',meta:'热菜现炒 · 营业至 21:30',eta:'30–40 分钟',freeAt:48,categories:['下饭热菜','一人食','汤羹']},
 {id:'sweet',name:'云朵甜品屋',short:'甜点',emoji:'🍰',mark:'SWEET',meta:'每日新鲜烘焙 · 营业至 22:00',eta:'25–35 分钟',freeAt:42,categories:['人气甜点','蛋糕','糖水']}
];
const TEA_MENU=[
...
let _teaDraft={contactId:null,cart:{},storeId:'mutea',direction:'outgoing',method:'delivery',address:'',note:''},_teaCustomProductId=null,_teaTrackTimers=[];
...
function renderTeaContacts(){const box=document.getElementById('teaContactList');if(!box)return;const role=_teaDraft.direction==='incoming'?'付款人':'收货人';document.getElementById('teaContactTitle').textContent=_teaDraft.direction==='incoming'?'谁请我':'送给谁';box.innerHTML=state.contacts.map(c=>`<button class="tea-contact-item ${String(_teaDraft.contactId)===String(c.id)?'selected':''}" data-cid="${c.id}">${avatarHtml(c.avatar,c.name,34)}<span>${escapeHtml(c.name)}</span><small>${role}</small></button>`).join('')||'<div class="tea-empty">请先添加联系人</div>';}
function renderTeaStores(){document.getElementById('teaStoreList').innerHTML=TEA_STORES.map(s=>`<button class="tea-store-card ${s.id===_teaDraft.storeId?'selected':''}" data-store-id="${s.id}"><span class="tea-store-emoji">${s.emoji}</span><span><strong>${escapeHtml(s.name)}</strong><small>${s.short} · ${s.eta}</small></span></button>`).join('');}
function renderTeaStore(){const s=currentTeaStore();document.getElementById('teaStoreName').textContent=s.name;document.getElementById('teaStoreMeta').innerHTML=`${escapeHtml(s.meta)}<br>预计 ${escapeHtml(s.eta)}送达`;document.getElementById('teaStoreBadge').textContent=`满 ¥${s.freeAt} 免配送费`;document.getElementById('teaStoreHero').dataset.mark=s.mark;document.getElementById('teaCategoryList').innerHTML=s.categories.map((x,i)=>`<button data-tea-category="${x}" class="${i===0?'selected':''}">${x}</button>`).join('');renderTeaStores();renderTeaMenu(s.categories[0]);}
...
function bindTeaOrder(){
 ...
 document.getElementById('teaStoreList')?.addEventListener('click',e=>{const b=e.target.closest('[data-store-id]');if(!b||b.dataset.storeId===_teaDraft.storeId)return;if(teaCartCount()&&!confirm('切换店铺会清空当前购物车，继续吗？'))return;_teaDraft.storeId=b.dataset.storeId;_teaDraft.cart={};renderTeaStore();updateTeaTotal();});
 ...
 document.getElementById('teaOrderDirection')?.addEventListener('click',e=>{const x=e.target.closest('[data-order-direction]');if(!x)return;_teaDraft.direction=x.dataset.orderDirection;document.querySelectorAll('[data-order-direction]').forEach(c=>c.classList.toggle('selected',c===x));renderTeaContacts();});
 ...
 document.getElementById('teaGoPayBtn')?.addEventListener('click',()=>{if(!teaCartCount()){alert('请先选择餐点');return;}if(!_teaDraft.contactId){alert(_teaDraft.direction==='incoming'?'请选择谁来请你':'请选择收货人');return;}if(_teaDraft.method==='delivery'){_teaDraft.address=document.getElementById('teaAddressInput').value.trim();if(!_teaDraft.address){alert('请填写配送地址');document.getElementById('teaAddressInput').focus();return;}safeSetItem(TEA_ADDRESS_KEY,_teaDraft.address);}_teaDraft.note=document.getElementById('teaNoteInput').value.trim();const incoming=_teaDraft.direction==='incoming',c=getContactById(_teaDraft.contactId);document.getElementById('teaPayStoreName').textContent=currentTeaStore().name;document.getElementById('teaPayAmount').textContent=`¥${teaTotal().toFixed(2)}`;document.getElementById('teaPayCurrentBalance').textContent=`¥${getWallet().balance.toFixed(2)}`;document.getElementById('teaPaySummary').textContent=`${teaCartCount()} 件 · ${_teaDraft.method==='delivery'?'配送':'到店自取'}`;document.getElementById('teaPayMethod').textContent=incoming?`${c?.name||'对方'}代付`:'零钱余额';document.getElementById('teaPayBalanceRow').classList.toggle('hidden',incoming);document.getElementById('teaConfirmPayBtn').textContent=incoming?`请${c?.name||'对方'}下单`:'确认支付';pushPage('page-teapay');});
 document.getElementById('backFromTeaPay')?.addEventListener('click',()=>popPage());
 document.getElementById('teaConfirmPayBtn')?.addEventListener('click',()=>{if(_walletActionBusy)return;const total=teaTotal(),incoming=_teaDraft.direction==='incoming',w=getWallet();if(!incoming&&w.balance<total){alert('余额不足，请先充值');return;}_walletActionBusy=true;const finish=()=>{const w2=getWallet();if(!incoming&&w2.balance<total){_walletActionBusy=false;alert('余额不足');popPage();return;}const now=Date.now(),accept=25000+secureRandomInt(65000),prepare=(6+secureRandomInt(9))*60000,travel=_teaDraft.method==='delivery'?(12+secureRandomInt(17))*60000:(2+secureRandomInt(5))*60000,s=currentTeaStore();const order={id:now,contactId:_teaDraft.contactId,storeId:s.id,storeName:s.name,direction:_teaDraft.direction,items:teaCartItems(),method:_teaDraft.method,address:_teaDraft.address,note:_teaDraft.note,total,createdAt:now,acceptedAt:now+accept,preparedAt:now+accept+prepare,completedAt:now+accept+prepare+travel,notified:false};if(!incoming){w2.balance-=total;w2.transactions.push({title:`${s.name}订单 · ${order.items.length}件`,amount:-total,ts:now});saveWallet(w2);}safeSaveJSON(TEA_ACTIVE_ORDER_KEY,order);scheduleTeaDeliveryNotification(order);_walletActionBusy=false;popPage();renderTeaTracking(order);if(incoming){const c=getContactById(order.contactId);if(c)replyWithTarot(String(c.id),String(c.id),`(你刚刚主动帮我在${s.name}点了外卖并已经付款，餐点是：${order.items.map(teaItemDescription).join('；')}。简短自然地告诉我，括号内容不要说出来)`,c.persona);}};if(incoming)finish();else runFaceIdSimulation(finish);});
...
function notifyTeaDelivery(order){if(!order||order.notified)return;order.notified=true;safeSaveJSON(TEA_ACTIVE_ORDER_KEY,order);const c=getContactById(order.contactId);if(!c)return;const detail=order.items.map(teaItemDescription).join('；');if(order.direction==='incoming'){addMessage(String(c.id),String(c.id),`配送通知：${order.storeName||'外卖'}订单已送达${order.address?' · '+order.address:''}`,{systemNote:true});replyWithTarot(String(c.id),String(c.id),`(你帮我点的外卖刚刚送达了，餐点是：${detail}。简短自然地提醒我趁热吃，括号内容不要说出来)`,c.persona);}else replyWithTarot(String(c.id),String(c.id),`(我送的外卖刚刚实际送达，你现在才收到。餐点是：${detail}。请自然回应)`,c.persona);}
...
function resumeTeaOrderNotification(){scheduleTeaDeliveryNotification(safeLoadJSON(TEA_ACTIVE_ORDER_KEY,null));}
function renderTeaTracking(order){
 ...
 document.getElementById('teaOrderSummary').innerHTML=`<strong>${escapeHtml(order.storeName||currentTeaStore(order.storeId).name)}</strong>${order.items.map(x=>`<span>${escapeHtml(teaItemDescription(x))}</span>`).join('')}<small>${order.direction==='incoming'?'对方已付款':'实付 ¥'+Number(order.total).toFixed(2)} · ${order.method==='delivery'?'配送至 '+escapeHtml(order.address||''):'到店自取'}</small>`;
