...
function renderTeaTracking(order){
 ...
 else if(!order.notified){order.notified=true;safeSaveJSON(TEA_ACTIVE_ORDER_KEY,order);const c=getContactById(order.contactId);if(c){const detail=order.items.map(teaItemDescription).join('；');if(order.direction==='incoming'){addMessage(String(c.id),String(c.id),`配送通知：${order.storeName||'外卖'}订单已送达${order.address?' · '+order.address:''}`,{systemNote:true});replyWithTarot(String(c.id),String(c.id),`(你帮我点的外卖刚刚送达了，餐点是：${detail}。简短自然地提醒我趁热吃，括号内容不要说出来)`,c.persona);}else replyWithTarot(String(c.id),String(c.id),`(我送的外卖刚刚实际送达，你现在才收到。餐点是：${detail}。请自然回应)`,c.persona);}}
}
